...
def main():
    ...
    with torch.no_grad(), open(args.result_file, 'w') as fout, open(args.result_file+'_real', 'w') as fout_real:
        for batch_idx, batch in enumerate(test_data_loader):
            ...
            if args.mode == 'emotion_recognition':
                res, confidence = model.inference(
                    feats,
                    feats_lengths,
                    target)
            else:
            ...
            for i, key in enumerate(keys):
                # print(res)
                # print(res.size())
                content=char_dict[res.item()]
                logger.info('{} {}'.format(key.split('/')[-1], content))
                fout.write('{} {}\n'.format(key.split('/')[-1], content))
